...
func (p *Parser) WithOptions(options ...ParserOption) {
	for _, op := range options {
		op(p)
	}
}
...
func (r *Parser) Bool(name string, value bool, usage string) *Parser {
	r.FlagSet.Bool(name, value, usage)
	r.boolOptionSet.Add(name)
	return r
}

// func (r *Parser) String(name, value, usage string) *Parser {
// 	r.FlagSet.String(name, value, usage)
// 	return r
// }

// func (r *Parser) Int(name string, value int, usage string) *Parser {
// 	r.FlagSet.Int(name, value, usage)
// 	return r
// }

// func (r *Parser) Int64(name string, value int64, usage string) *Parser {
// 	r.FlagSet.Int64(name, value, usage)
// 	return r
// }

// func (r *Parser) Float64(name string, value float64, usage string) *Parser {
// 	r.FlagSet.Float64(name, value, usage)
// 	return r
// }
...
func (r *Parser) ParseArgsCmd(boolOptionals ...string) {
	...
	for i, arg := range os.Args[start:] {
		...
		args[i] = strings.ReplaceAll(args[i], " ", string(space))
...
func classifyArguments(cmd string, boolOptionals ...string) (*cw.LinkedList[string], []string, []string, []string) {
	...
	positionals.WithNormalization(func(s string) string {
		return strings.ReplaceAll(s, string(space), " ")
	})
	var keys []string
	var boolKeys []string
	var vals []string
